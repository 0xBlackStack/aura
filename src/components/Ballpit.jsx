...
class W {
  constructor(e) {
    ...
    this.previousPositionData = new Float32Array(3 * e.count).fill(0);
  ...
  update(e) {
    ...
    this.previousPositionData.set(this.positionData);
    let r = 0;
    if (t.controlSphere0) {
      r = 1;
      F.fromArray(s, 0);
...
const X = {
  count: 200,
  colors: [0xff6b6b, 0x4ecdc4, 0x45b7d1, 0xf7dc6f, 0xbb8fce, 0xf8c471, 0x85c1e9, 0xd7bde2],
...
function createBallpit(e, t = {}) {
  ...
  i.onBeforeRender = e => {
    if (!c) {
      s.update(e);
      // check scoring with throttle
      if (Date.now() - lastScoreTime > 100) {
        const threshold = s.config.maxY * 0.7;
        for (let idx = 0; idx < s.config.count; idx++) {
          const y = s.physics.positionData[3 * idx + 1];
          const prevY = s.physics.previousPositionData[3 * idx + 1];
          if (y > threshold && prevY <= threshold) {
            onScore?.();
            lastScoreTime = Date.now();
            break; // only one score per check
          }
        }
      }
    }
...
const Ballpit = ({ className = '', followCursor = true, onScore, ...props }) => {
  ...
  useEffect(() => {
    ...
    spheresInstanceRef.current = createBallpit(canvas, { followCursor, onScore, ...props });
